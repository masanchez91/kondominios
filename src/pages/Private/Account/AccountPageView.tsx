...
const getCardType = (number: string) => {
  ...
  if (visaRegex.test(number)) return <FaCcVisa className="text-blue-600 text-3xl" />;
  if (masterCardRegex.test(number)) return <FaCcMastercard className="text-yellow-600 text-3xl" />;
  if (amexRegex.test(number)) return <FaCcAmex className="text-green-600 text-3xl" />;
  return null;
};
...
function PricingCard({ title, desc, price, options, onSelect, isCurrentPlan, isAnnual }: PricingCardPropsType) {
  return (
    <div className={`bg-white shadow-lg rounded-lg p-6 border-2 ${isCurrentPlan ? 'border-gray-300' : 'border-magenta-500'} transform transition duration-300 hover:scale-105`}>
        <h2 className="text-xl font-bold text-magenta-700 capitalize mb-1">{title}</h2>
        ...
        <p className="text-4xl font-bold text-magenta-700 flex items-center">
            {price[0]}{price[1]}
            <span className="text-lg font-bold text-magenta-500 self-end">/{price[2]}</span>
        </p>
        {isAnnual && ( // Mostrar mensaje de ahorro solo si el plan es anual
            <p className="text-sm text-magenta-500 mt-1">Ahorras un 20% con el plan anual</p>
        )}

        {/* Lista de características */}
        <ul className="flex flex-col gap-3 mb-6">
            {options.map((option, key) => (
            <li key={key} className="flex items-center gap-3 text-magenta-700">
                <span className="h-5 w-5 bg-magenta-700 text-white flex items-center justify-center rounded-full">{option.icon}</span>
  ...
  const cards = [
    {
      ...
      options: [
        ...
        { icon: "✅", info: "Envío de avisos y notificaciones a residentes" },
        { icon: "✅", info: "Cobranza manual de cuotas y pagos (no automatizada)" },
        { icon: "✅", info: "Soporte" },
      ],
      isCurrentPlan: false,
    },
    {
      ...
      options: [
        { icon: "✅", info: "Generación de reportes financieros y de mantenimiento" },
        { icon: "✅", info: "Envío de avisos y notificaciones a residentes" },
    ...
    },
    {
      ...
      options: [
        ...
        { icon: "✅", info: "Control de acceso mediante códigos QR" },
        { icon: "✅", info: "Cobranza automatizada de cuotas y pagos" },
        { icon: "✅", info: "Soporte prioritario y asistencia personalizada" },
      ],
      ...
      <section className="py-24 px-8 bg-magenta-50">
        <div className="max-w-7xl mx-auto bg-white py-10 px-6 md:px-24 rounded-lg shadow-xl">
          <div className="text-center mb-10">
            <h3 className="text-lg font-bold text-magenta-700">Planes de Precios</h3>
            ...
            <p className="text-magenta-600 max-w-screen-xl mx-auto">
                Compara los planes y elige el que mejor se ajuste a tu presupuesto y objetivos. Nuestra solución facilita la gestión, optimiza procesos y mejora la experiencia de los residentes.
            </p>
          </div>

          <div className="flex justify-center items-center gap-4 mb-8">
            <span className={`text-purple-700 font-medium ${!isAnnual ? 'font-bold' : 'opacity-50'}`}>Mensual</span>
            <div
            ...
            <span className={`text-purple-700 font-medium ${isAnnual ? 'font-bold' : 'opacity-50'}`}>Anual</span>
          </div>

          <div className="grid gap-x-10 gap-y-8 md:grid-cols-2 max-w-5xl mx-auto">
            {cards.map(({ title, desc, options, price, isCurrentPlan }, key) => (
              <PricingCard
                key={key}
                title={title}
                desc={desc}
          ...
          <p className="mt-10 text-magenta-600 text-center">
            Todos los planes incluyen actualizaciones ilimitadas y soporte. 🚀
          </p>
        </div>
      </section>
    );
    ...
    return (
      <section className="py-24 px-8 bg-magenta-50">
        <div className="max-w-3xl mx-auto bg-white py-10 px-6 md:px-12 rounded-lg shadow-xl">
          <h2 className="text-2xl font-bold text-pink-700 mb-4">Detalles del Plan</h2>
          <p className="mb-6 text-magenta-600">
            Has seleccionado el plan <strong>{selectedPlan}</strong>. Revisa los detalles y completa el proceso de pago.
          </p>
          
          <div className="border rounded-lg p-6 bg-gray-50">
            <h3 className="text-lg font-bold text-magenta-700 mb-4">{selectedPlan}</h3>
            <p className="text-magenta-600 mb-4">Colaboración en equipo para proyectos web profesionales.</p>
            
            <div className="mb-6">
              <p className="text-magenta-700 font-bold">1 miembro pagado</p>
              <p className="text-4xl font-bold text-magenta-700">$19</p>
              <p className="text-magenta-600">Total mensual: $19</p>
            </div>

            <div className="mb-6">
              <p className="text-magenta-700 font-bold">$19 por miembro /mes</p>
              <ul className="text-magenta-700 space-y-2">
                {cards.find(card => card.title === selectedPlan)?.options.map((option, key) => (
            ...
            {showPaymentForm ? (
              <div className="mb-6">
                <h3 className="text-lg font-bold text-magenta-700 mb-4">Añadir método de pago</h3>
                <div className="space-y-4">

                <div className="relative">
                    <label className="block text-magenta-700">Número de tarjeta</label>
                    <input
                        type="text"
                        value={cardNumber}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-magenta-700">Fecha de expiración</label>
                      <input type="text" maxLength={7}
              value={expirationDate}
              onChange={(e) => setExpirationDate(formatExpirationDate(e.target.value))} className="w-full p-2 border border-magenta-500 rounded-lg" placeholder="MM / YY" />
                    </div>
                    <div>
                      <label className="block text-magenta-700">Código de seguridad</label>
                      <input type="text"
                        value={securityCode}
                  ...
                  <div>
                    <label className="block text-magenta-700">Nombre completo</label>
                    <input type="text" className="w-full p-2 border border-magenta-500 rounded-lg" placeholder="Nombre completo" />
                  </div>
              ...
              <div className="mb-6">
                <p className="text-magenta-700 font-bold">Método de pago</p>
                <p className="text-magenta-600">Se requiere al menos un método de pago para completar el proceso.</p>
                <button
